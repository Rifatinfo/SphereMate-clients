...
const BidRequests = () => {
  ...
  // const {category,deadline,max_price,min_price,job_title,description} = jobs;
  useEffect( () => {
  ...
  const getData = async () => {
      const {data} = await axios(`http://localhost:9000/bid-request/${user?.email}`);
      setBids(data);
  }

  // handleStatus 
  const handleStatus = async (id, preStatus , status) =>{
    if(preStatus === status) return console.log('Sorry Do Not Match')
